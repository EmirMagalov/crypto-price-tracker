...
from schemas.price import Price as Db_Price
...
@router.get("/limit/", response_model=List[Db_Price])
def get_limit_prices(
    ticker: str = Query(..., description="Ticker валюты"), db: Session = Depends(get_db)
):
    prices = (
        db.query(Price)
...
@router.get("/", response_model=List[Db_Price])
def get_prices(
    ticker: str = Query(..., description="Ticker валюты"), db: Session = Depends(get_db)
):
...
@router.get("/latest/", response_model=Db_Price)
...
@router.get("/filter/", response_model=Db_Price)
def get_prices_by_date(
    ticker: str = Query(...), date_ts: int = Query(...), db: Session = Depends(get_db)
):
